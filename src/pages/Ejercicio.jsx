...
function ExerciseForm() {
  ...
  const getActivities = async () => {
    try {
      ...
      console.log("activitiesUs data:", data);
      setActivities(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleSearch = (event) => {
    const searchTerm = event.target.value.trim().toLowerCase(); // Obtener el término de búsqueda en minúsculas

    // Realizar la concatenación de activities y activitiesUs
    const mergedActivities = activitiesUs.concat(activities);
    console.log(mergedActivities);
    // Filtrar los datos según el término de búsqueda
    const newData = mergedActivities.filter((item) => {
      // Verificar si item.nameActivity está definido y es una cadena antes de llamar a toLowerCase()
      return (
        item.nameActivity &&
        typeof item.nameActivity === "string" &&
        item.nameActivity.toLowerCase().includes(searchTerm)
      );
    });

    // Actualizar el estado dataAuxActivities con los resultados de la búsqueda
    setDataAuxActivities(newData);
    console.log(newData); // Mostrar los resultados de la búsqueda en la consola

    // Limpiar el estado activity
    setActivity([]);
  };

...
export default function Comidas() {
  ...
  const handleCheckboxSMAE = (e, record) => {
    if (e.target.checked) {
      ...
      setAlimentsSelect(
        alimentsSelect.filter((food) => food.id !== record.idSMAE)
      );
    }
    ...
    try {
      ...
      await Promise.all(
        alimentsSelect.map(async (aliment) => {
          const { error } = await supabase.from("BancoAlimentos").insert({
            uid: user.id,
            food: aliment.name,
  ...
  const fetchSMAE = async () => {
    try {
      ...
      const { data: foodSMAE, error } = await supabase
        .from("SMAE")
        .select(
          "idSMAE,food,tipoAlimento(idTipoalimento,food), unidadesMedida(idUnidadMedida,name),carbohydrates, portionamount"
        );
      if (error) {
        throw error;
      }
      const available_foods = foodSMAE.filter(
        (food) => !aliments.some((meat) => meat.food === food.food)
      );
